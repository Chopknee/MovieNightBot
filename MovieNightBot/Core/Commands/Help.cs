    ...
    public class Help : ModuleBase<SocketCommandContext> {
        ...
        public async Task GetHelp() {//Because we could all use some from time to time.
            try {
                ...
                builder.AddField("m!user_vote_count [Number]", ":no_entry: Sets the number of movies users will be allowed to vote on. This cannot be greater than the number of movie options that shows on a vote.");
                ...
                builder.AddField("m!set_movie_time [Number]", ":no_entry: Sets the hour when the movie will be watched. The hour is in UTC time zone, so convert accordingly. Valid range is 0 - 23 This time shows at the bottom of the vote embed.");
                Embed embed = builder.Build();
                await Context.User.SendMessageAsync("You Need Some Help", embed: embed).ConfigureAwait(false);
            } catch (HttpException ex) {
                await Context.Channel.SendMessageAsync("I can't send you the help message right now, you might have me blocked or something.");
            } catch (Exception ex) {
